...
EM.player = (function ($) {
    ...
    var firstLoad = true, //indicate if player loaded already or not
        ...
        context = { //where the player window is playing
            ...
            init: function () {
                ...
                this.isMobile = window.location.href.indexOf("/mobile/") !== -1;
            }
        },
        options = {
    ...
    // initialize player
     function init ( id ){
         ...
         $('#scene')
             ...
             .on('transitionDone', setParallaxEffectIfExist);
    ...
    function setPlayerSidebarButtons() {
        ...
        $('#pl-watermark-indp').toggleClass('hidden', !permissions.showLogo || ezContentType == 1 || ezContentType == null);
    ...
    // set parallax effect to elements if it has.
    function runParallax() {
        var $slide = $('.current-active-slide');
        if ($slide.length < 1) {
            setTimeout(runParallax, 20);
        }
        var scaleFactor = 0, underfined;
        ...
        if (scaleFactor == 0 && $('.em-parallax').length > 0) {
            setTimeout(runParallax, 20);
        } else {
            $('.em-parallax').parallax();
        }
    }

    function setParallaxEffectIfExist() {
        if (EM.player.context.isEditor && $(window.frameElement).is(':visible') || !EM.player.context.isEditor) {
            var $slides = EM.compatibility.isCompatible() ? $('.slide [class*="effect_parallax"]') : $('.slide_g [class*="effect_parallax"]');
            if ($('.em-parallax').length) {
                $('.em-parallax').parallax('destroy');
                $slides.removeClass("em-parallax");
            }
            if ($slides.length && $slides.parallax('hasSlides')) {
                $slides.parallax('destroyAll');
            }
            $('.current-active-slide [class*="effect_parallax_none"]').removeClass("em-parallax");
            $('.current-active-slide [class*="effect_parallax_scroll"]').addClass("em-parallax").attr("data-em-parallax", "scroll").children().addClass("parallax-slider");
            $('.current-active-slide [class*="effect_parallax_fixed"]').addClass("em-parallax").attr("data-em-parallax", "fixed").children().addClass("parallax-slider");
            $('.current-active-slide [class*="effect_parallax_zoom"]').addClass("em-parallax").attr("data-em-parallax", "zoom").children().addClass("parallax-slider");
            $('.current-active-slide').scrollTop(0);
            runParallax();
        }
    ...
    function stateCalled(e){  
        ...
        //if this is mobile and was click on link with slidenum parameter
        if (EM.player.context.isMobile) {
            if (e !== undefined && e.type == "hashchange" && window.location.hash.indexOf("slidenum=") !== -1) {
                presentSlideNumber = setSlideNumberFromURL(true);
                $(window).trigger('changeslide', { 'detail': { slideNumber: presentSlideNumber, isAutoplay: isAutoplay } });
                window.location.hash = "";
            }
        }
        if (presentSlideNumber != pastSlideNumber) {
            ...
            if (EM.isLayoutsPlayer && presentSlideNumber == 2) {
                if (slidesSize < 3) {
                    presentSlideNumber = 1;
                }
    ...
    function getSlideNumberNoHash() {
        return presentSlideNumber;
    }

    // return slide number (as integer); isLink - bool parameter, by default = false, if true - call from mobile after click on link(have parameter slidenum) 
    function setSlideNumberFromURL(isLink) {

        var hash = window.location.hash,
            slideNumber;

        hash = hash.substr(1).replace('/','');//trim out the # and remove "/" if this is mobile or tablet device
        ...
        if (options !== undefined && !options.navigateViaHashChange) {
            return presentSlideNumber;
        }
    ...
    function isHideByContentType(nameOfControl) {
        switch (ezContentType) {
            ...
            case 6:
                return true;
            case 3:  //show arrows with timeline
            ...
            case 4:
            case 5:
            case 8: //show arrows and menu(hamburger)
                switch (nameOfControl) {
                    case "hidebuttons":
                        return false;
                    case "hidecontrols":
